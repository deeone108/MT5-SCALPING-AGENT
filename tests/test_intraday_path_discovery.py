...
def test_zero_path_is_missing_and_future_cannot_change_causal_features():
    times=pd.date_range('2020-01-01',periods=180,freq='min',tz='UTC'); prices=np.ones(180); prices[:100]+=np.arange(100)*.0001; frame=pd.DataFrame({'time':times,'open':prices,'high':prices,'low':prices,'close':prices,'volume':1})
    base=build_observations('EURUSD',frame); altered=frame.copy(); altered.loc[120:,'close']+=1; later=build_observations('EURUSD',altered)
