"""Evaluate frozen Strategy 15 on the isolated 2019-2023 development split.
...
STRATEGY_NAME = "compression_expansion_controlled_continuation"
RESEARCH_ID = "strategy_15_compression_expansion_controlled_continuation_v1"
...
PROMOTION_GATE_ID = "strategy_15_economic_gate_v1"
FIXED_VOLUME_LOTS = 1.0
INITIAL_BALANCE = 10_000.0
BLOCK_BOOTSTRAP_SEED = 20_260_824
BLOCK_BOOTSTRAP_SAMPLES = 10_000
DEFAULT_REPORT_PATH = Path(
    "reports/strategy15/eurusd_2019_2023_strategy15_development.json"
)
...
SCENARIOS = (
    CostScenario("base", BASE_COST_MODEL_ID, 2.0, 1.0, 2.0, 0.7),
    CostScenario("stress", STRESS_COST_MODEL_ID, 4.0, 2.0, 2.0, 1.0),
)
...
class _Strategy15ManifestBinding:
...
def parse_arguments(arguments: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run frozen Strategy 15 at unit exposure under its base and stress "
...
def _run_development(
    ...
    *,
    ...
    expected_manifest = build_run_manifest(
        run_kind="strategy15_development_unit_economics",
        execution_timestamp=datetime.now(UTC),
        strategies={STRATEGY_NAME: _Strategy15ManifestBinding},
        strategy_parameters={
        ...
        relevant_code_paths=(
            Path(__file__),
            args.registry_path,
            Path("docs/STRATEGY_15_RESEARCH_BRIEF.md"),
        ),
        random_seed=BLOCK_BOOTSTRAP_SEED,
    ...
    for scenario in SCENARIOS:
        if scenario.name in completed:
            print(f"Skipping completed Strategy 15 {scenario.name} scenario", flush=True)
            continue
        print(
            f"Starting Strategy 15 {scenario.name} scenario at fixed 1.0 lot; "
    ...
    metrics = _gate_metrics(evaluations)
    gate_report = strategy15_gate_report(metrics, governance.gate)
    ...
    report = {
        "purpose": (
            "prospectively frozen Strategy 15 development evaluation at constant "
        ...
        "results": results,
        "stop_rule": (
            "If any primary or eligible block-bootstrap gate fails, mark Strategy 15 "
...
def _validate_exact_development_scope(
    ...
    if start.tzinfo is None or end.tzinfo is None:
        raise SplitIsolationError("Strategy 15 development boundaries must be timezone-aware")
    ...
    if normalized != (DEVELOPMENT_START, DEVELOPMENT_END):
        raise SplitIsolationError(
            "Strategy 15 development requires exactly [2019-01-01, 2024-01-01); "
...
def _load_frozen_governance(
    ...
    if record is None or record.research_id != RESEARCH_ID:
        raise ValueError("the frozen Strategy 15 registry record is missing")
    fingerprint = preregistration_fingerprint(registry, STRATEGY_NAME)
    if fingerprint != EXPECTED_PREREGISTRATION_FINGERPRINT:
        raise ValueError("Strategy 15 preregistration fingerprint changed; refuse evaluation")
    if record.status is not ResearchStatus.DEVELOPMENT:
        raise ValueError("Strategy 15 must be in DEVELOPMENT before archive access")
    if record.decision is not ResearchDecision.UNDECIDED:
        raise ValueError("Strategy 15 development must have an UNDECIDED decision")
    if record.implementation != EXPECTED_IMPLEMENTATION:
        raise ValueError("Strategy 15 registry implementation does not match frozen code")
    if record.experiments_performed:
        raise ValueError("first Strategy 15 development run requires zero prior experiments")
    if record.permitted_development_dataset_id != DEVELOPMENT_DATASET_ID:
        raise ValueError("Strategy 15 development dataset identity changed")
    if record.required_broker_cost_model_id != BASE_COST_MODEL_ID:
        raise ValueError("Strategy 15 base cost-model identity changed")
    if record.promotion_gate_id != PROMOTION_GATE_ID:
        raise ValueError("Strategy 15 promotion gate identity changed")
    ...
    if (
        ...
        raise ValueError("Strategy 15 dataset specification is not exact Dukascopy 2019-2023")
    ...
    if not isinstance(gate, EconomicPromotionGate):
        raise ValueError("Strategy 15 requires its frozen economic promotion gate")
...
def _validate_strategy_defaults(frozen: Mapping[str, object]) -> None:
    defaults = asdict(CompressionExpansionControlledContinuationConfig())
    for key, value in defaults.items():
        ...
        if expected != observed:
            raise ValueError(f"Strategy 15 code default drifted from frozen parameter {key}")
...
def _load_development_candles(
    ...
    *,
    ...
    if source_for_range(start, end) != "dukascopy":
        raise SplitIsolationError("Strategy 15 accepts only the Dukascopy provider")
    ...
    if not candles.empty and (
        ...
        raise SplitIsolationError("archive returned candles outside Strategy 15 development")
    return candles
...
def _new_strategy(
    ...
    *,
    strategy_type: type[CompressionExpansionControlledContinuationStrategy] = (
        CompressionExpansionControlledContinuationStrategy
    ),
) -> CompressionExpansionControlledContinuationStrategy:
...
) -> tuple[BacktestResult, CompressionExpansionControlledContinuationStrategy]:
...
def _gate_metrics(
    ...
    return Strategy15GateMetrics(
        ...
        mfe_exceedance_ratio=(
            sum(value > 3.0 for value in mfe) / len(mfe) if mfe else None
        ),
...
def _diagnostics_document(diagnostics: object) -> dict[str, object]:
    return {
        "evaluated_expansion_bars": int(getattr(diagnostics, "evaluated_expansion_bars")),
        "eligible_setup_count": int(getattr(diagnostics, "eligible_setup_count")),
        ...
        "reused_setup_block_count": int(getattr(diagnostics, "reused_setup_block_count")),
...
def _load_completed_evaluations(
    ...
    for row in results:
        ...
        if not isinstance(payload, dict):
            raise ValueError(f"Strategy 15 detail is not an object: {path}")
...
def _validate_checkpoint_results(
    ...
    for row in results:
        ...
        if scenario not in expected or scenario in seen:
            raise ValueError(f"checkpoint contains invalid Strategy 15 scenario: {scenario!r}")
        seen.add(scenario)
