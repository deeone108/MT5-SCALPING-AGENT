"""Evaluate frozen Strategy 16 on the isolated 2019-2023 development split.
...
BLOCK_BOOTSTRAP_SEED = 20_260_830
BLOCK_BOOTSTRAP_SAMPLES = 10_000
DEFAULT_REPORT_PATH = Path(
    "reports/strategy16/eurusd_2019_2023_strategy16_development.json"
)
...
SCENARIOS = (
    CostScenario("base", BASE_COST_MODEL_ID, 4.0, 2.0, 2.0, 1.0),
    CostScenario("stress", STRESS_COST_MODEL_ID, 10.0, 5.0, 2.0, 1.9),
)
...
def parse_arguments(arguments: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run frozen Strategy 16 at unit exposure under its base and stress "
...
def _run_development(
    ...
    *,
    ...
    expected_manifest = build_run_manifest(
        run_kind="strategy16_development_unit_economics",
        ...
        relevant_code_paths=(
            Path(__file__),
            args.registry_path,
            Path("docs/STRATEGY_16_RESEARCH_BRIEF.md"),
        ),
        random_seed=BLOCK_BOOTSTRAP_SEED,
    ...
    for scenario in SCENARIOS:
        if scenario.name in completed:
            print(f"Skipping completed Strategy 16 {scenario.name} scenario", flush=True)
            continue
        print(
            f"Starting Strategy 16 {scenario.name} scenario at fixed 1.0 lot; "
    ...
    gate_report = strategy16_gate_report(metrics, governance.gate)
    ...
    report = {
        "purpose": (
            "prospectively frozen Strategy 16 development evaluation at constant "
        ...
        "results": results,
        "stop_rule": (
            "If any primary or eligible block-bootstrap gate fails, mark Strategy 16 "
...
def _validate_exact_development_scope(
    ...
    if start.tzinfo is None or end.tzinfo is None:
        raise SplitIsolationError("Strategy 16 development boundaries must be timezone-aware")
    ...
    if normalized != (DEVELOPMENT_START, DEVELOPMENT_END):
        raise SplitIsolationError(
            "Strategy 16 development requires exactly [2019-01-01, 2024-01-01); "
...
def _load_frozen_governance(
    ...
    if record is None or record.research_id != RESEARCH_ID:
        raise ValueError("the frozen Strategy 16 registry record is missing")
    fingerprint = preregistration_fingerprint(registry, STRATEGY_NAME)
    if fingerprint != EXPECTED_PREREGISTRATION_FINGERPRINT:
        raise ValueError("Strategy 16 preregistration fingerprint changed; refuse evaluation")
    if record.status is not ResearchStatus.DEVELOPMENT:
        raise ValueError("Strategy 16 must be in DEVELOPMENT before archive access")
    if record.decision is not ResearchDecision.UNDECIDED:
        raise ValueError("Strategy 16 development must have an UNDECIDED decision")
    if record.implementation != EXPECTED_IMPLEMENTATION:
        raise ValueError("Strategy 16 registry implementation does not match frozen code")
    if record.experiments_performed:
        raise ValueError("first Strategy 16 development run requires zero prior experiments")
    if record.permitted_development_dataset_id != DEVELOPMENT_DATASET_ID:
        raise ValueError("Strategy 16 development dataset identity changed")
    if record.required_broker_cost_model_id != BASE_COST_MODEL_ID:
        raise ValueError("Strategy 16 base cost-model identity changed")
    if record.promotion_gate_id != PROMOTION_GATE_ID:
        raise ValueError("Strategy 16 promotion gate identity changed")
    ...
    if (
        ...
        raise ValueError("Strategy 16 dataset specification is not exact Dukascopy 2019-2023")
    ...
    if not isinstance(gate, EconomicPromotionGate):
        raise ValueError("Strategy 16 requires its frozen economic promotion gate")
...
def _validate_strategy_defaults(frozen: Mapping[str, object]) -> None:
    defaults = asdict(ScheduledMacroShockConfig())
    for key, value in defaults.items():
        ...
        if expected != observed:
            raise ValueError(f"Strategy 16 code default drifted from frozen parameter {key}")
...
def _load_development_candles(
    ...
    *,
    ...
    if source_for_range(start, end) != "dukascopy":
        raise SplitIsolationError("Strategy 16 accepts only the Dukascopy provider")
    ...
    if not candles.empty and (
        ...
        raise SplitIsolationError("archive returned candles outside Strategy 16 development")
    return candles
...
def _new_strategy(
    ...
    *,
    strategy_type: type[ScheduledMacroShockContinuationStrategy] = (
        ScheduledMacroShockContinuationStrategy
    ),
) -> ScheduledMacroShockContinuationStrategy:
...
) -> tuple[BacktestResult, ScheduledMacroShockContinuationStrategy]:
...
def _load_completed_evaluations(
    ...
    for row in results:
        ...
        if not isinstance(payload, dict):
            raise ValueError(f"Strategy 16 detail is not an object: {path}")
...
def _validate_checkpoint_results(
    ...
    for row in results:
        ...
        if scenario not in expected or scenario in seen:
            raise ValueError(f"checkpoint contains invalid Strategy 16 scenario: {scenario!r}")
        seen.add(scenario)
