...
def main():
    ...
    costs=json.loads(Path('reports/cross_pair_feasibility/roboforex_ecn_cross_pair_cost_models.json').read_text())['models']; opp={p:opportunity_to_cost(primary[primary.pair==p],float(costs[p]['base']['round_trip_cost_pips']),float(costs[p]['stress']['round_trip_cost_pips'])) for p in PAIRS}
