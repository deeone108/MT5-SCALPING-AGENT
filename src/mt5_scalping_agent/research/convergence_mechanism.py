...
def path_metrics(residuals,times,positions,horizon=60,step_minutes=5):
    ...
    return {'status':np.where(valid,'AVAILABLE','MISSING_EXACT_PATH'),'valid':valid,'time_to_75':first(ratio<=.75),'time_to_50':first(ratio<=.5),'time_to_25':first(ratio<=.25),'maximum_widening':np.where(valid,np.max(safe,1),np.nan),'time_to_max_widening':np.where(valid,offsets[np.argmax(safe,1)],np.nan)}
