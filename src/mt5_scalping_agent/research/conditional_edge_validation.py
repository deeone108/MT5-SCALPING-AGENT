...
def attach_path(events,m1,pair):
    out=events.copy(); close=m1.set_index("time").close; arr=close.to_numpy(float); loc=close.index.get_indexer(pd.DatetimeIndex(out.event_time)); offsets=np.array(PATH_HORIZONS); pos=loc[:,None]+offsets; valid=(loc>=0)&(pos[:,-1]<len(arr)); out=out.loc[valid].copy(); loc=loc[valid]; pos=pos[valid]; ref=arr[loc]; direction=out.direction.to_numpy(float)[:,None]; values=arr[pos]; moves=direction*(values-ref[:,None])/pip_size(pair)
    for i,h in enumerate(PATH_HORIZONS): out[f"path_{h}m_pips"]=moves[:,i]
    positions=loc[:,None]+np.arange(1,61); movement=direction*(arr[positions]-ref[:,None])/pip_size(pair); out["mfe_60m_pips"]=np.max(movement,1); out["mae_60m_pips"]=np.min(movement,1); out["time_to_mfe_minutes"]=np.argmax(movement,1)+1; out["time_to_mae_minutes"]=np.argmin(movement,1)+1; out["mfe_before_mae"]=out.time_to_mfe_minutes<out.time_to_mae_minutes; out["mae_before_mfe"]=out.time_to_mae_minutes<out.time_to_mfe_minutes; return out
